//Sports
import Sports_1 from "./Assets/FieldWiseA_Img/Sports_1.png";
import Sports_2 from "./Assets/FieldWiseA_Img/Sports_2.png";
import Sports_3 from "./Assets/FieldWiseA_Img/Sports_3.png";
import Sports_4 from "./Assets/FieldWiseA_Img/Sports_4.png";
import Sports_5 from "./Assets/FieldWiseA_Img/Sports_5.png";
import Sports_6 from "./Assets/FieldWiseA_Img/Sports_6.png";

//Technology
import Technology_1 from "./Assets/FieldWiseA_Img/Technology_1.png";
import Technology_2 from "./Assets/FieldWiseA_Img/Technology_2.png";
import Technology_3_6 from "./Assets/FieldWiseA_Img/Technology_3.png";
import Technology_4 from "./Assets/FieldWiseA_Img/Technology_4.png";
import Technology_5 from "./Assets/FieldWiseA_Img/Technology_5.png";
import Technology_6 from "./Assets/FieldWiseA_Img/Technology_6.png";
import Technology_8 from "./Assets/FieldWiseA_Img/Technology_8.png";
import Technology_9 from "./Assets/FieldWiseA_Img/Technology_9.png";
import Technology_10 from "./Assets/FieldWiseA_Img/Technology_10.png";

//Cultural
import Cultural_1 from "./Assets/FieldWiseA_Img/Cultural_1.png";
import Cultural_2 from "./Assets/FieldWiseA_Img/Cultural_2.png";
import Cultural_3 from "./Assets/FieldWiseA_Img/Cultural_3.png";
import Cultural_4 from "./Assets/FieldWiseA_Img/Cultural_4.png";
import Cultural_5 from "./Assets/FieldWiseA_Img/Cultural_5.png";

const Sports = {
  name: "Sports",
  description:
    "Physical recreation through games and sports has been an important part of the Institute since its inception. Games like football, cricket and hockey were introduced around 1870 followed closely by squash and boating. The tradition continues, as students of the institute keep making use of the marvellous sports facilities to excel in sports and games. In order to motivate the students and to get them interested in sports, a special scheme of NSO (National Sports Organisation) has been running from the session 2007-08.",
  Achievements_List: [
    {
      id: 1,
      dep: "In the 54th inter IIT sports meet women’s Volleyball team won gold medal while Men won bronze medal in Table tennis and squash tournament.",
      link: "",
      date: "23/12/2019",
    },
    {
      id: 2,
      dep: "In the 53rd sports meet, the women's Basketball and volleyball team won gold medal while Sumit yadav was awarded as the best player of the Hockey.",
      link: "",
      date: "19/12/2018",
    },
    {
      id: 3,
      dep: "In the 52nd sports meet Aman Pal set a new record in Hammer Throw and Trishna K Chengappa in 100m sprint.",
      link: "",
      date: "20/12/2017",
    },
    {
      id: 4,
      dep: "In the 50th sports meet men’s basketball, hockey , Football and squash won the gold medal while the silver medal in Tennis. Women won a Bronze medal in Basketball. Also Roorkee came second in the Overall General Championship (Men).",
      link: "",
      date: "25/12/2015",
    },
    {
      id: 5,
      dep: "In 49th sports meet IIT Roorkee came FIrst in the Overall General Championship (Women).",
      link: "",
      date: " 28/12/2014",
    },
    {
      id: 6,
      dep: "In the 48th sports meet Roorkee won gold in Basketball (women) , Football(men), Squash and swimming , volleyball (women) and silver medal in Badminton(men) , Basketball(men).",
      link: "",
      date: "18/12/2013",
    },
  ],
  Bubble_List: [
    {
      image: Sports_1,
    },
    {
      image: Sports_2,
    },
    {
      image: Sports_3,
    },
    {
      image: Sports_4,
    },
    {
      image: Sports_5,
    },
    {
      image: Sports_6,
    },
  ],
};

const Technology = {
  name: "Technology",
  description:
    "Technology-that's the middle name of our institute IIT Roorkee.And with full awareness of the above fact, a number of student technical societies flourish in the institute. These societies organise various events from time to time-quizzes,debates,seminars,guest lectures and many more. They thus provide the students with a great platform to enhance their technical skills",
  Achievements_List: [
    {
      id: 1,
      dep: "KNOx achieved 1 Rank in Design Section in Virtuals Round among all IITs.",
      link: "https://new.iitr.ac.in/mechanical/team-knox/",
      date: "2017",
    },
    {
      id: 2,
      dep: "Students from the iGEM IITR have participated for the 1st time and won a gold medal in iGEM (synthetic-biology based competition).",
      link: "https://www.facebook.com/111403760578466/posts/200789174973257/",
      date: "27/11/2020",
    },
    {
      id: 3,
      dep: "33 GSoC selection (Globally highest).",
      link: "https://opensource.googleblog.com/2021/06/google-summer-of-code-2021-student-stats.html",
      date: "2021",
    },
    {
      id: 4,
      dep: "InfoSec IITR regularly participate in competitive hacking competitions and have been consistently ranked among the top 3 CTF teams in India.",
      link: "http://watchout.iitr.ac.in/Freshman_Guide_To_IITR.pdf",
      date: "",
    },
    {
      id: 5,
      dep: "KNOx achievements in BAJA include 2nd place among 326 teams in CAE category (awarded by ANSYS Inc.).",
      link: "https://iitr.ac.in/departments/ME/pages/Events+Student_Sections+KNOx.html",
      date: "2013",
    },
    {
      id: 6,
      dep: "IITR team in ICPC world final since 7 years.",
      link: "",
      date: "",
    },
    {
      id: 7,
      dep: "Highest number of GSoC selection since 2018.",
      link: "https://developers.google.com/open-source/gsoc/resources/stats",
      date: "",
    },
    {
      id: 8,
      dep: "IIT Roorkee was one of the premier institutions slated to receive PARAM 10000 supercomputer.",
      link: "https://www.iitr.ac.in/centers/CC/pages/Resources+Param_1000.html",
      date: "",
    },
    {
      id: 9,
      dep: "Roorkee Motorsports team achieved 3rd position in Formula Green 2020.",
      link: "https://alumni.iitr.ac.in/iitrnews/formulagreen",
      date: "FEB,2020",
    },
    {
      id: 10,
      dep: "ArIES IITR team has qualified for the final among top 10 teams in MathWorks MiniDrone Competition - EMEA (Europe Middle-East Asia) 2021.",
      link: "https://www.facebook.com/503218879766649/posts/4333020526786446/",
      date: "23/09/2021",
    },
  ],
  Bubble_List: [
    {
      image: Technology_1,
    },
    {
      image: Technology_2,
    },
    {
      image: Technology_3_6,
    },
    {
      image: Technology_4,
    },
    {
      image: Technology_5,
    },
    {
      image: Technology_6,
    },
    {
      image: Technology_3_6,
    },
    {
      image: Technology_8,
    },
    {
      image: Technology_9,
    },
    {
      image: Technology_10,
    },
  ],
};

const Cultural = {
  name: "Cultural",
  description:
    "The cultural society provides a stage to showcase your talents and helps to break away from the shackles of innumerable projects, assignments and quizzes. The various groups that are the limbs of the cultural society help you in embracing your inner artists. Be it bringing various characters to life, exhibiting new dance moves, composing new melodies or being a part of brainstorming sessions, you will always find a group of your interest.",
  Achievements_List: [
    {
      id: 1,
      dep: "IIT Roorkee stands 2nd in Inter IIT Cult meet 2018.",
      link: "",
      date: "30/12/2018",
    },
    {
      id: 2,
      dep: "Many videos of Cinesec IITR has achieved more than million views on YouTube.",
      link: "",
      date: "",
    },
    {
      id: 3,
      dep: "DebSoc IITR team were semifinalists at the inter IIT Debating Championship and Arnav from the team adjudged the final debate of the tournament.",
      link: "",
      date: "",
    },
    {
      id: 4,
      dep: "The work of Kshitij has also been appreciated by the former President of India, Dr. A.P.J. Abdul Kalam and Dr. Murali Manohar Joshi.",
      link: "",
      date: "",
    },
    {
      id: 5,
      dep: "IITR team stands 1st in 51 hours filmmaking at Inter IIT Cult meet 2019.",
      link: "",
      date: "28/12/19",
    },
  ],
  Bubble_List: [
    {
      image: Cultural_1,
    },
    {
      image: Cultural_2,
    },
    {
      image: Cultural_3,
    },
    {
      image: Cultural_4,
    },
    {
      image: Cultural_5,
    },
  ],
};

const FeildData = [Sports, Technology, Cultural];

export default FeildData;
